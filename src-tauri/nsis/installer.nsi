...
Function PageReinstall
  ...
  ${ElseIf} $R0 = 1
    ${If} $WixMode <> 1
      ; Locus: upgrade-compatible NSIS installs update in place.
      StrCpy $UpdateMode 1
      Abort
    ${EndIf}
...
Function PageLeaveReinstall
  ...
  ${ElseIf} $R0 = 1 ; Upgrading
    ; Locus: keep upgrade path on update mode.
    StrCpy $UpdateMode 1
    Goto reinst_done
...
Function RestorePreviousInstallLocation
  ReadRegStr $4 SHCTX "${MANUPRODUCTKEY}" ""
  ; Locus: read install locations from historical metadata keys.
  ${If} $4 == ""
