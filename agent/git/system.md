...
- Use `knowledge_query` followed by `read` to inspect registered knowledge. Keep bash commands that touch `Locus/knowledge` limited to git operations such as `git status`, `git diff`, `git add`, `git restore --staged`, and `git commit`.
