...
/** Character offsets and text are preserved exactly; format adapters own normalization. */
export interface DocumentTextEditOperation {
  oldString: string;
  newString: string;
...
export interface DocumentTextHunk {
  start: number;
  end: number;
  oldText: string;
  newText: string;
...
export interface DocumentTextConflict {
  local: DocumentTextHunk;
  remote: DocumentTextHunk;
}

export interface DocumentTextRebaseResult {
  text: string;
  remotePreferredText: string;
  conflicts: DocumentTextConflict[];
  localHunks: DocumentTextHunk[];
  remoteHunks: DocumentTextHunk[];
}
...
export function buildDocumentTextHunks(baseValue: string, nextValue: string): DocumentTextHunk[] {
  const base = baseValue;
  const next = nextValue;
  if (base === next) return [];

  const hunks: DocumentTextHunk[] = [];
  let baseOffset = 0;
...
function hunksAreEqual(left: DocumentTextHunk, right: DocumentTextHunk): boolean {
  return left.start === right.start
    && left.end === right.end
    && left.newText === right.newText;
}

function hunksOverlap(left: DocumentTextHunk, right: DocumentTextHunk): boolean {
...
function applyHunks(base: string, hunks: DocumentTextHunk[]): string {
...
export function rebaseDocumentText(
  baseValue: string,
  localValue: string,
  remoteValue: string,
): DocumentTextRebaseResult {
  const base = baseValue;
  const local = localValue;
  const remote = remoteValue;
  const localHunks = buildDocumentTextHunks(base, local);
  const remoteHunks = buildDocumentTextHunks(base, remote);
  ...
  const conflicts: DocumentTextConflict[] = [];
...
export function buildDocumentTextEditOperations(
  baseValue: string,
  nextValue: string,
): DocumentTextEditOperation[] {
  const base = baseValue;
  const next = nextValue;
  const hunks = buildDocumentTextHunks(base, next);
