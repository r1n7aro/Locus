...
const props = defineProps<{
  ...
  registerPaneTarget: (paneId: string, element: HTMLElement) => void;
  recordPaneScroll: (paneId: string, event: Event) => void;
}>();
...
function registerGroupElement(value: Element | ComponentPublicInstance | null): void {
  if (props.node.kind === "group" && value && "nodeType" in value && value.nodeType === 1) {
    props.registerPaneTarget(props.node.paneId, value as HTMLElement);
  }
}
...
<template>
  ...
  <section
    ...
    @scroll.capture.passive="recordPaneScroll(node.paneId, $event)"
  >
    <div
