...
describe("ConsoleSettings layout", () => {
  ...
  it("adds a log export action with a .log save dialog", () => {
    ...
    expect(source).toContain("saveDebugConsoleLogExport(filePath)");
    expect(source).toContain(':disabled="isExporting"');
    expect(source).toContain('t("settings.console.export")');
    expect(service).not.toContain("formatDebugConsoleEntriesForLogExport");
    ...
    expect(rustCommand).toContain("sink.export_to(&export_path)");
