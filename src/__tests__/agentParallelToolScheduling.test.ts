...
describe("agent parallel tool scheduling safety", () => {
  it("serializes mutating rounds under the process workspace write lock", () => {
    ...
    const release = agent.indexOf("drop(workspace_round_guard.take())", checkpoint);
  ...
  it("preconfirms before locking and isolates reentrant tool kinds", () => {
    ...
    expect(agent).toContain("user-input tools must run alone");
