...
describe("plugin Skill package", () => {
  it("exposes /plugin as the single command entry", () => {
    ...
    expect(skill).toContain("locate the View Skill with `knowledge_query`");
    expect(skill).toContain("locate the Create Skill workflow with `knowledge_query`");
    expect(skill).toContain("locate the [publishing workflow](publish.md) with `knowledge_query`");
    expect(skill).toContain("`read` its returned physical path");
    expect(skill).toContain("ask_user_question");
    expect(skill).toContain("Present the export metadata in one concise block");
    expect(skill).toContain("Call `plugin_export` after the latest proposal is approved.");
    expect(skill).toContain("record the approved values, including user edits, in `userApproval`");
    expect(publish).toContain("present the registry metadata in one concise block");
  ...
  it("keeps plugin tools aligned with the natural-language workflow", () => {
    ...
    const pluginSkill = read("skills/plugin/SKILL.md");
    ...
    expect(pluginSkill).toContain("include plugin Views by id");
  });
});
