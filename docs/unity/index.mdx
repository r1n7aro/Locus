...
编辑器扩展也可以调用公开接口，并指定引用进入焦点会话或新会话：
...
省略参数时使用 `FocusedSession`。`NewSession` 会切换到一个新会话草稿后附加当前 Unity 选择；当前已存在的会话保持不变。

本地文件和文件夹使用 `LocusFileAttachment` 显式传入：
...
路径必须在调用时存在。无效路径会被忽略；全部路径无效时接口返回 `false`。
