...
Skill documents are the exception: the model can only discover a skill through its injected structure line, so this level shows as `No Injection` on skills — it turns auto recall off entirely, leaving only the slash command.

Defaults differ by type: Design and Plan documents default to `L0 - Path Injection`, Skill documents to `L1 - Summary Injection`, and Memory and Reference documents to `Search Only`. Memory's three preset modules ship pre-tuned for their roles (Mistake Notebook at L2, User Preferences at L3, Project Understanding at L0); see [Design and Memory](/en/knowledge/memory-and-design).
