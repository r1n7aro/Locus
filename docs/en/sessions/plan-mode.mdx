...
Type `/plan` in the composer with your task description (available in Unity sessions only). A sticky banner appears at the top of the conversation reading `Plan mode (read-only)`, with an `Exit plan mode` button to leave manually at any time.
