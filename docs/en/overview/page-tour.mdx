...
Before sending a message, check that the `Asset Database` and `Unity Editor Connection` indicators (top-left) are both green.
...
- **Locus-tagged files**: knowledge and config files owned by the Agent. If you don't want them under version control, add their paths to `.gitignore`.
...
- **Lexical indexing** (on by default): keyword-based exact text matching.
