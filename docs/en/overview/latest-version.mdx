...
description: "Locus v0.8.4 (2026-09-23)"
...
- [Windows x64](https://github.com/r1n7aro/Locus/releases/download/v0.8.4/locus_0.8.4_x64-setup.exe)
- [Windows x64 (system Python/Git)](https://github.com/r1n7aro/Locus/releases/download/v0.8.4/locus_0.8.4_x64-without_embed_python_git-setup.exe)
...
- `unity_yaml_read` accepts `reader: "default"` on asset roots to bypass matching Skill readers. Child paths can resolve serialized fields hidden from the Inspector through the live Editor.
- Manually refreshing a Codex subscription now updates usage limits and the available model list together.
...
- Added global search for Knowledge documents and active or archived sessions. Press `Ctrl+F` in Development to search titles and content across the projects shown in the workspace, with previews, modification times, keyboard navigation, and configurable search sources.
- Added managed Git worktrees and reusable Unity project pools. The Python SDK can create, import, acquire, release, and target checkouts, and manage their Unity Editor lifecycle, including headless operation.
...
- Added the local `/garbage-collection` command to reclaim unused space in the session and current project databases while preserving their records.
...
- Removed View templates and the old `view_*` tool registrations; View creation and operations now use `execute_typescript` and the shared SDK. Existing View packages remain readable.
